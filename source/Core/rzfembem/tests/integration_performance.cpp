...
TEST_F(IntegrationPerformanceTest, SimpleExpressionIntegration) {
    std::cout << "\n=== Simple Expression Integration Performance ===" << std::endl;
    ...
    for (int interval : intervals) {
        auto test_name = "Constant_integration_" + std::to_string(interval) + "_intervals";
...
TEST_F(IntegrationPerformanceTest, PolynomialExpressionIntegration) {
    std::cout << "\n=== Polynomial Expression Integration Performance ===" << std::endl;
...
TEST_F(IntegrationPerformanceTest, CompoundExpressionIntegration) {
    std::cout << "\n=== Compound Expression Integration Performance ===" << std::endl;
    ...
    Expression complex_compound(complex_outer, {
...
TEST_F(IntegrationPerformanceTest, CoordinateMappingIntegration) {
    std::cout << "\n=== Coordinate Mapping Integration Performance ===" << std::endl;
...
TEST_F(IntegrationPerformanceTest, DerivativeIntegration) {
    ...
    Expression base_expr("u1*u1 + u2*u2 + u3*u3", {"u1", "u2", "u3"});  // Add variable list
    std::vector<std::string> vars = {"u1", "u2", "u3"};
...
TEST_F(IntegrationPerformanceTest, ElementBasisIntegration) {
    ...
    benchmark_function("FEM1D_vertex_integration", [&]() {
        ...
        for (const auto& shape_func : vertex_exprs) {
            ...
            integrate_over_simplex(product, fem1d->get_barycentric_names(), nullptr, 50);
        }
    });
    ...
    benchmark_function("FEM2D_vertex_integration", [&]() {
        ...
        for (const auto& shape_func : vertex_exprs) {
            Expression product = shape_func * test_expr;
            integrate_over_simplex(product, fem2d->get_barycentric_names(), nullptr, 50);
        }
    });
    ...
    benchmark_function("FEM3D_vertex_integration", [&]() {
        ...
        for (const auto& shape_func : vertex_exprs) {
            Expression product = shape_func * test_expr;
            integrate_over_simplex(product, fem3d->get_barycentric_names(), nullptr, 30);
        }
    });
}
...
TEST_F(IntegrationPerformanceTest, ArithmeticOperationsIntegration) {
    std::cout << "\n=== Arithmetic Operations Integration Performance ===" << std::endl;
    ...
    benchmark_function("Complex_arithmetic_integration", [&]() {
        auto complex_expr = (expr1 + expr2) * (expr1 - expr2) / (expr1 + Expression::constant(1.0));
...
TEST_F(IntegrationPerformanceTest, IntegrationIntervalScaling) {
    std::cout << "\n=== Integration Interval Scaling Performance ===" << std::endl;
    
    Expression test_expr("u1*u2 + u2*u3 + u3*u1", {"u1", "u2", "u3"});  // Add variable list
    std::vector<std::string> vars = {"u1", "u2", "u3"};
    ...
    for (int interval : intervals) {
        ...
        std::cout << "  -> " << static_cast<int>(ops_per_sec) << " integrations/second" << std::endl;
    }
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "\nTotal test suite execution time: " << total_duration << " ms" << std::endl;
